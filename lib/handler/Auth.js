...
export const UserLogin = {
  ...
  Output: db.CustomerSchema.omit({ id: true, dibuat: true }),
  url: '/user/customer'
}

handle.build(UserLogin, async ({ exec }, { cookie }) => {
  
  const [sessionValue] = await exec(`\
  SELECT c.nama,c.nohp,c.alamat FROM session s
  LEFT JOIN user u ON u.id = s.user_id AND u.type = 'user'
  LEFT JOIN customer c ON c.id = u.subjek
  WHERE sessionId = ?
  `, [cookie], UserLogin.Output)
...
handle.build(UserRegister, async ({ exec, insert }, { username, passwd, alamat, nama, nohp }) => {
  ...
  const user = await insert(table.user.insert(),[username, passwd, 'customer', customer.insertId])
