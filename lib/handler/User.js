export const GetDriver = {
  Input: z.object({ driver_id: z.number(), tipe: z.enum(['driver','kurir']).default('driver'), nocheck: z.literal(true).optional() }),
  Output: db.DriverSchema,
  url: '/driver'
}


handle.build(GetDriver, async ({exec}, { driver_id, tipe, nocheck }) => {
  const [driver] = await exec(
    table.driver.select() + ' where driver_id=? and tipe = ? limit 1',
    [driver_id, tipe]
  )
  
  if (!nocheck && tipe == 'kurir') {
